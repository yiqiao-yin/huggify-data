...
class DataFrameUploader:
    ...
    def verify_dataframe(self):
        if "questions" not in self.df.columns or "answers" not in self.df.columns:
            raise ValueError(
                "The dataframe must have columns named 'questions' and 'answers'."
            )
    ...
    def process_data(self):
        if self.verify_dataframe():
            ...
            raw_content_qa = [
                f"### Human: {q} ### Assistant: {a}"
                for q, a in zip(raw_content_questions, raw_content_answers)
            ]
    ...
    def push_to_hub(self):
        if self.dataset_dict:
            api = HfApi()
            create_repo(
                self.repo_name, token=self.hf_token, private=False
            )  # Set private=True if you want it to be a private dataset
            ...
            raise ValueError(
                "DatasetDict is not created. Ensure process_data() is called after dataframe verification."
            )
